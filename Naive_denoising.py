...
class Naive_denoising(object):
	def __init__(self, dimension_of_observation, dimension_of_unknown, noise_variance, show_observation = False, show_figure = False, save_figure = False):
		if (dimension_of_unknown - 1) % (dimension_of_observation - 1) != 0:
			print("Dimensions of observation and unknown are not matched.")
			sys.exit(0)

		self.coarse_factor = (dimension_of_observation - 1) / (dimension_of_unknown - 1)
	...
	def Gu(self, u):
		Gu = np.linspace(0, 1, dimension_of_observation)
		for i in range(dimension_of_observation):
	...
	def Plot_MAP(self):
		...
		if (self.show_observation): plt.plot(self.observation_ordinate, self.observation, 'r')
		plt.plot(self.observation_ordinate, self.MAP, label = legend_text)
		plt.legend()
	...
	def Plot_CM(self, sample_size, beta):
		...
		if (self.show_observation): plt.plot(self.observation_ordinate, self.observation, 'r')
		plt.plot(self.observation_ordinate, self.Posterior_Mean, label = legend_text)
		plt.legend()
...
if __name__ == '__main__':
	...
	# Sample_example = Naive_denoising(dimension_of_observation = 45, dimension_of_unknown = 89, noise_variance = 0.01, show_observation = False, show_figure = False, save_figure = False)
