...
class Naive_denoising(object):
	def __init__(self, dimension_of_observation, dimension_of_unknown, noise_variance, show_observation = False, show_figure = False, save_figure = False):
		...
		self.coarse_factor = (dimension_of_unknown - 1) / (dimension_of_observation - 1)
	...
	def TV_norm(self, u): return self.Lambda * LA.norm(np.delete((u), 0) - np.delete(u, self.dimension_of_observation - 1), 1)
	def Gaussian_norm(self, u): return LA.norm(np.dot(self.inv_sqrt_prior_covariance, u)) ** 2 / 2
	def pV_norm(self, u): return self.Lambda * (self.dimension_of_observation - 1)**(self.p - 1) * LA.norm(np.delete((u),0) - np.delete(u, self.dimension_of_observation - 1), self.p)**self.p
	...
	def Metropolis_Hastings(self, sample_size = 1000, beta = 0.1):
		...
		for i in range(sample_size):
			...
			random_walk = np.random.multivariate_normal(np.zeros(self.dimension_of_unknown), 
				self.noise_variance * np.eye(self.dimension_of_unknown))
